...
export function getStrainData(
  ...
  const description = `${name} is listed as ${tierDesc} in the ${typeLabel} lane at Gas Junction Cannabis${thcLine}. Compare the current product page, menu notes, weight, and posted price before choosing.`;

  const metaDescription = `${name} at Gas Junction Cannabis in Toronto. Compare the current ${tierDesc} listing, product notes, and staff guidance before visiting.`;
